...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_section'], $_POST['stud_id'])) {
    ...
    if ($teacher_result && pg_num_rows($teacher_result) > 0) {
        ...
        if (in_array($teach_no, [12, 13, 14, 15]) && in_array($section_id, [8, 9, 10, 11])) {
            pg_query_params($conn, "
                UPDATE STUDENT 
                SET gradelvl_id = 8 
                WHERE stud_id = $1
    ...
    header("Location: sciGrade8Infos.php?id=" . $stud_id);
    exit;
}
...
<head>
    <meta charset="UTF-8">
    <title>Grade 8 SSC - Student Info</title>
...
<div class="Info">
    <h1> Grade 8 SSC Student Information</h1>
</div>
...
<div class="cards-body">
    <table class="table table-bordered">
        ...
        if ($result && pg_num_rows($result) > 0) {
            ...
            $sectionResult = pg_query($conn, "SELECT section_id, section_name FROM SECTION WHERE gradelvl_id IN (8, 14)");
            ...
            echo "</td>
                <td>
                    <a href='sciGrade8Infos.php?delete=$stud_id' class='btn btn-danger' onclick='return confirm(\"Are you sure you want to delete this student?\")'><i class='fas fa-trash-alt'></i></a>
                </td>
            </tr>";
        } else {
