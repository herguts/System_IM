...
if (isset($_GET['delete'])) {
    ...
    header("Location: sciGrade10.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_section'], $_POST['stud_id'])) {
    ...
    if ($teacher_result && pg_num_rows($teacher_result) > 0) {
        ...
        if (in_array($teach_no, [22, 23, 24, 25]) && in_array($section_id, [18, 19, 20, 21])) {
            pg_query_params($conn, "
                UPDATE STUDENT 
                SET gradelvl_id = 10 
                WHERE stud_id = $1
    ...
    header("Location: sciGrade10Infos.php?id=" . $stud_id);
    exit;
}
...
<head>
    <meta charset="UTF-8">
    <title>Grade 10 SSC - Student Info</title>
...
<div class="Info">
    <h1> Grade 10 SSC Student Information</h1>
</div>
...
<div class="cards-body">
    <table class="table table-bordered">
        ...
        if ($result && pg_num_rows($result) > 0) {
            ...
            echo "</td>
                <td>
                    <a href='sciGrade10Infos.php?delete=$stud_id' class='btn btn-danger' onclick='return confirm(\"Are you sure you want to delete this student?\")'><i class='fas fa-trash-alt'></i></a>
                </td>
            </tr>";
        } else {
